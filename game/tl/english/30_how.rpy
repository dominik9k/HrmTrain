# TODO: Translation updated at 2015-07-13 21:36

# game/30_how.rpy:18
translate english howtoplay_59d24d4e:

    # l "Привет интернет-извращугам!"
    l "Hello, pervs of the internet!"

# game/30_how.rpy:20
translate english howtoplay_009106a5:

    # a5 "Следи за языком, сучка!"
    a5 "Watch it, brat!"

# game/30_how.rpy:24
translate english howtoplay_666f6e51:

    # l "Хах...?"
    l "Huh...?"

# game/30_how.rpy:26
translate english howtoplay_cef21509:

    # a6 "Что я тебе говорил о слове на букву \"и\"?"
    a6 "What did I tell you about using the \"p\" word?"

# game/30_how.rpy:30
translate english howtoplay_5747f407:

    # l "Эм... Использовать его как можно чаще..?"
    l "Em... Use it as often as possible?"

# game/30_how.rpy:34
translate english howtoplay_38c624be:

    # a7 "{size=+7}Нет!{/size}"
    a7 "{size=+7}Wrong!{/size}"

# game/30_how.rpy:39
translate english howtoplay_8be61403:

    # l "Гх!"
    l "Ght!"

# game/30_how.rpy:43
translate english howtoplay_a8553285:

    # a7 "{size=+7}Мы не используем его! Никогда!{/size}"
    a7 "{size=+7}We don't use it! Ever!{/size}"

# game/30_how.rpy:47
translate english howtoplay_99dd8419:

    # l "Потому что самый самый большой здесь папочка?"
    l "Because daddy is the biggest perv there is?"

# game/30_how.rpy:49
translate english howtoplay_0c16bec4:

    # a6 "Гх!"
    a6 "Ght!"

# game/30_how.rpy:50
translate english howtoplay_227d1f12:

    # a6 "Тебе понравилось сниматься в \"Тренер Принцессы\"?"
    a6 "Did you enjoy starring in \"Princess Trainer\"?"

# game/30_how.rpy:54
translate english howtoplay_8dde85ca:

    # l "Лучшее событие в моей жизни!"
    l "It was the best!"

# game/30_how.rpy:56
translate english howtoplay_b04841cd:

    # a1 "Хочешь попасть в \"Золотое издание\"?"
    a1 "Want to be in the \"Gold Edition\" as well?"

# game/30_how.rpy:60
translate english howtoplay_f2f5f7c5:

    # l "!!!"
    l "!!!"

# game/30_how.rpy:63
translate english howtoplay_c931db92:

    # l "Дамы и гопода, добро пожаловать в обучающий режим \"Тренера Гермионы\"."
    l "Ladies and gentlemen, welcome to the \"Hermione Trainer\" tutorial."

# game/30_how.rpy:65
translate english howtoplay_d2b5fc7a:

    # a1 "Умница, девочка."
    a1 "Attagirl."

# game/30_how.rpy:73
translate english howtoplay_14bd4678:

    # l "Хм...?"
    l "Hm...?"

# game/30_how.rpy:74
translate english howtoplay_298383c3:

    # l "Ты хочешь прослушать обучение снова?"
    l "You want to hear the tutorial again?"

# game/30_how.rpy:76
translate english howtoplay_dbc66370:

    # l "Хм...."
    l "Hm...."

# game/30_how.rpy:78
translate english howtoplay_a3a14bb1:

    # l "Ты не смущен?"
    l "You are not very bright, are you?"

# game/30_how.rpy:80
translate english howtoplay_7492697c:

    # l "Хм..."
    l "Hm..."

# game/30_how.rpy:83
translate english howtoplay_b5ae1d03:

    # l "*Хихикает!*"
    l "*Giggle!*"

# game/30_how.rpy:86
translate english howtoplay_46b12eb1:

    # l "Ты хочешь, чтобы я учила тебя топлесс?"
    l "You want me to give you the tutorial topless?"

# game/30_how.rpy:95
translate english howtoplay_b83ee881:

    # l "Океюшки!"
    l "Alrighty!"

# game/30_how.rpy:110
translate english howtoplay_93939895:

    # l "Ты скучный..."
    l "You're boring..."

# game/30_how.rpy:112
translate english howtoplay_851f54ed:

    # l "Ладно, пофиг..."
    l "Well, whatever..."

# game/30_how.rpy:119
translate english howtoplay_48730f05:

    # l "Вот краткий перечень вещей, которые стоит помнить..."
    l "Here is a short list of things to keep in mind..."

# game/30_how.rpy:124
translate english howtoplay_184a7442:

    # l "Гермиона захочет продавать сексуальные услуги в обмен на очки факультета, когда Гриффиндор отстает."
    l "Hermione will only be willing to sell sexual favors in exchange for house points when house of Gryffindor is falling behind."

# game/30_how.rpy:129
translate english howtoplay_8bdaf106:

    # l "Дружба с профессором Снейпом увеличит количество очков, получаемых Слизереном."
    l "Making friends with Professor Snape will make him trust you more, and will increase the rate at which Slytherin house gets the points."

# game/30_how.rpy:135
translate english howtoplay_2e505438:

    # l "Чтение образовательных книг позволит тебе зарабатывать, но это по желанию."
    l "Reading educational books makes earning money easier, but is not mandatory."

# game/30_how.rpy:140
translate english howtoplay_d8eeccc6:

    # l "Покупка одной и той же сексуальной услуги может привести к разным последствиям, в зависимости от того, как далеко Гермиона зашла в своих тренировках."
    l "Buying the same sexual favor again could lead to a different outcome depending on how far along Hermione is in her training."

# game/30_how.rpy:145
translate english howtoplay_a0aa07dd:

    # l "Все услуги разделены на две группы: \"приватные услуги\" и\"публичные услуги\"."
    l "All favors are organized into two groups: \"Personal favors\" and \"Public favors\"."

# game/30_how.rpy:146
translate english howtoplay_e847ad0b:

    # l "Приватные услуги оказываются в кабинете и не сильно влияют на репутацию Гермионы."
    l "Personal favors take place in the office and don't affect Hermione's reputation much."

# game/30_how.rpy:147
translate english howtoplay_3c7d6474:

    # l "Публичные услуги оказываются во время уроков за пределами экрана."
    l "Public favors take place off-screen, during classes."

# game/30_how.rpy:148
translate english howtoplay_78768d73:

    # l "Каждая публичная услуга, не считая последней, имеет девять концовок."
    l "Every public favor, except for the last one, has nine different outcomes."

# game/30_how.rpy:149
translate english howtoplay_1e4fa5ad:

    # l "Кстати, несмотря на то, что покупка приватных услуг - необходима для тренировки Гермионы, публичные услуги не обязательны."
    l "Also, buying personal favors is essential for Hermione's training, but public favors are optional."

# game/30_how.rpy:155
translate english howtoplay_e4b6e180:

    # l "Если обращаться с ней плохо, то настроение Гермиона ухудшится, она может обидеться и стать очень неподатливой..."
    l "Treating Hermione poorly will worsen her mood and could make her very uncooperative..."

# game/30_how.rpy:156
translate english howtoplay_0a3b6a76:

    # l "Она остынет со временем, но ты можешь ускорить процесс, если подаришь ей что-нибудь..."
    l "She will cool off with time, but you can speed up the process by buying her something nice..."

# game/30_how.rpy:161
translate english howtoplay_0700b2f7:

    # l "Здесь нет временных ограничений. Так что можешь играть в нее столько дней, сколько захочешь."
    l "And there is no timelimit. So feel free to take as many days to comlete the game as you want."

# game/30_how.rpy:172
translate english howtoplay_a0fd6f67:

    # a1 "Ладно, этого хватит..."
    a1 "Alright, that's enough..."

# game/30_how.rpy:176
translate english howtoplay_84438caa:

    # l "Как я справилась?"
    l "How did I do?"

# game/30_how.rpy:178
translate english howtoplay_8262d4ba:

    # a1 "Ты отлично поработала. Хорошая девочка."
    a1 "You did well. Good girl."

# game/30_how.rpy:183
translate english howtoplay_82269e70:

    # l "Хе-хе-хе. Лола хорошая девочка!"
    l "Me-he-he. Lola is a good girl!"

# game/30_how.rpy:187
translate english howtoplay_bd32c5a2:

    # l "А что я получу?"
    l "What do I get?"

# game/30_how.rpy:189
translate english howtoplay_10dc6c37:

    # a1 "А что ты хочешь?"
    a1 "What you want?"

# game/30_how.rpy:192
translate english howtoplay_7492697c_1:

    # l "Хм..."
    l "Hm..."

# game/30_how.rpy:195
translate english howtoplay_9c3c13c2:

    # l "Мы можем сделать сцену изнасилования со мной в \"Золотом издании\"?"
    l "Can we have a rape scene with me in the \"Gold Edition\"?"

# game/30_how.rpy:197
translate english howtoplay_0b0d380e:

    # a6 "Не испытывай мое терпения, девочка."
    a6 "Don't test my patience, girl."

# game/30_how.rpy:202
translate english howtoplay_14316114:

    # l "Извини, папочка."
    l "Sorry, daddy."

# game/30_how.rpy:205
translate english howtoplay_1d4601a6:

    # a5 "............"
    a5 "............"

# game/30_how.rpy:211
translate english howtoplay_039435fa:

    # a1 "Что здесь происходит?"
    a1 "What is going on here?"

# game/30_how.rpy:215
translate english howtoplay_1a9a2485:

    # l "Упс!"
    l "Yikes!"

# game/30_how.rpy:217
translate english howtoplay_0dc3e989:

    # a1 "Что я говорил тебе о раздевании перед незнакомцами?"
    a1 "What did I tell you about exposing yourself to complete strangers?"

# game/30_how.rpy:220
translate english howtoplay_84d442d8:

    # l "Это важная часть взросления?"
    l "It's an important part of growing up?"

# game/30_how.rpy:222
translate english howtoplay_9733b25a:

    # a6 "Нет!"
    a6 "Wrong!"

# game/30_how.rpy:228
translate english howtoplay_821ad6df:

    # l "Папочка, мне так жаль!"
    l "Daddy, I'm so sorry!"

# game/30_how.rpy:229
translate english howtoplay_2951afe8:

    # l "Этот случайный чувак из интернета заставил меня, я клянусь!"
    l "That mean random Internet person forced me, I swear!"

# game/30_how.rpy:231
translate english howtoplay_6ca8595b:

    # a1 "Обучение закончено."
    a1 "This tutorial is over."

# game/30_how.rpy:236
translate english howtoplay_7a2b54dc:

    # l "Хе-хе! Ты попался!"
    l "He-he! You got busted!"

# game/30_how.rpy:239
translate english howtoplay_885be8c3:

    # l "И, это..."
    l "And that's that..."

# game/30_how.rpy:241
translate english howtoplay_37e65c5e:

    # l "Может быть в следующий раз?"
    l "Did you get it this time?"

# game/30_how.rpy:251
translate english abouttrainer_f9420893:

    # dev "Итак, сейчас перед вами \"Тренер Ведьм: Русская редакция\"."
    dev "Alright, what you see before you is \"Witch Trainer: Russian Editon\"."

# game/30_how.rpy:252
translate english abouttrainer_bc7a80c0:

    # dev "Наща дружная команда взяла за основу небольшую, но очень приятную игру от Акабура."
    dev "Our team, took as basis the small, but very pleasant game from Akabur."

# game/30_how.rpy:253
translate english abouttrainer_3278626e:

    # dev "Сейчас от нее остались главным образом идеи и контент."
    dev "At the moment all that rest of the main game is ideas and the content."

# game/30_how.rpy:254
translate english abouttrainer_46c2a06b:

    # dev "Код переписан и улучшен, а самое главное добавлено много нового и улучшено старое."
    dev "The code was enhanced and rewritten, but the best part of it, there is a lot of new content and improved the old one."

# game/30_how.rpy:255
translate english abouttrainer_c72136a4:

    # dev "Добавлены новые персонажи, новые ивенты и предметы. Перечислять можно долго."
    dev "There are new characters, new items and events. There's to much to list it all."

# game/30_how.rpy:256
translate english abouttrainer_1ce649a0:

    # dev "Более того, мы, в отличие от Акабура, не считаем проект законченным и постоянно совершенствуем его."
    dev "Moreover, in caparison with Akabur, we don't consider the project finished and constantly trying to improve it."

# game/30_how.rpy:257
translate english abouttrainer_f4e383ab:

    # dev "А еще, перевели игру на русский язык, чтобы русским людям было приятно играть."
    dev "And we did an English/Russian Translation for the game, so everybody can enjoy it."

# game/30_how.rpy:258
translate english abouttrainer_f3c34f3c:

    # dev "Мы делаем это совершенно бесплатно."
    dev "We are doing it completely free of charge."

# game/30_how.rpy:259
translate english abouttrainer_488c5528:

    # dev "По традиции, сохранены слова Акабура, далее вы можете с ними ознакомиться."
    dev "By tradition, you can read the original text from Akabur down below."

# game/30_how.rpy:261
translate english abouttrainer_7980a438:

    # a1 "Хм... Посмотрим..."
    a1 "Hm... Let's see..."

# game/30_how.rpy:262
translate english abouttrainer_4ba61919:

    # a1 "Я начал работу над \"Тренером Гермионы\" сразу после релиза \"Тренера Принцессы\"."
    a1 "I started to work on \"Hermione Trainer\" right after the release of \"Princess Trainer\"."

# game/30_how.rpy:263
translate english abouttrainer_a710cac5:

    # a1 "У меня была идея создать маленькую милую игру, на разработку которой уйдет не больше, чем пара месяцев."
    a1 "In my mind I had an idea for a cute little game the development of which should not have taken me longer then a couple of months."

# game/30_how.rpy:264
translate english abouttrainer_ebd0d479:

    # a1 "Как мы все знаем, это заняло у меня больше полугода..."
    a1 "As we all know now it actually took me longer then half a year..."

# game/30_how.rpy:265
translate english abouttrainer_2bb589b1:

    # a1 "И несмотря на бесконечные компромиссы, мне пришлось просто-напросто закончить разработку, чтобы эта чертовщина не убила у меня еще больше времени."
    a1 "But despite countless compromises I had to make it simple to prevent the damn thing taking even longer to develop."

# game/30_how.rpy:266
translate english abouttrainer_70c314c8:

    # a1 "Порой работать над игрой было весело..."
    a1 "Working on this game was quite fun at times..."

# game/30_how.rpy:267
translate english abouttrainer_5ad10c1f:

    # a1 "Но также были и трудности..."
    a1 "But it was also challenging..."

# game/30_how.rpy:268
translate english abouttrainer_9eb9032c:

    # a1 "Иногда мне приходилось бороться с некотрыми идеями и игровыми механиками, которые не хотели работать правильно..."
    a1 "Sometimes I had to wrestle with some ideas and gameplay mechanics that didn't want to work properly..."

# game/30_how.rpy:269
translate english abouttrainer_d033e21b:

    # a1 "Также работа над этой игрой рассказала мне много о моих способностях как разработчик игр и моих слабостях."
    a1 "Also working on this game taught me a lot about my abilities as a game developer and of my limitations."

# game/30_how.rpy:270
translate english abouttrainer_cf9a02ad:

    # a2 "Мне почти кажется, что я должен получить медаль или грамоту за это."
    a2 "I almost feel like I should be getting an achievement diploma or something."

# game/30_how.rpy:271
translate english abouttrainer_8cb9d261:

    # a1 "Ладно, я сваливаю в мой следующий проект. {size=-4}(\"Тренер Принцессы Золотое Издание\"){/size}"
    a1 "Well, I am off to my next project now. {size=-4}(\"Princess Trainer Gold Edition\"){/size}"

# game/30_how.rpy:272
translate english abouttrainer_b54b8388:

    # a1 "Спасибо за поддержку, парни. И я надеюсь, что эта игра сделала ваши будни хоть чуточку светлее."
    a1 "Thank you for supporting me, guys. And I hope this game will brighten up your day a bit."

# game/30_how.rpy:273
translate english abouttrainer_755acacc:

    # a2 "До следующего раза..."
    a2 "Until next time..."

# game/30_how.rpy:284
translate english faq_28bd540b:

    # a1 "Привет. Меня зовут Акабур. Я создатель игры, которая легла в основу данного проекта, и я здесь, чтобы ответить на твои вопросы."
    a1 "Hello. My name is Akabur. I am the creator of this game and I'm here to answer your questions."

# game/30_how.rpy:288
translate english faq2_c996317a:

    # a1 "Ну, есть несколько способов..."
    a1 "Well, there are few ways to do that..."

# game/30_how.rpy:289
translate english faq2_b7849af1:

    # a1 "Самый простой способов написать мне (akaburfake2@yahoo.com) и дать мне знать, что тебе понравилась игра."
    a1 "The easiest way would be to contact me (akaburfake2@yahoo.com) and let me know that you enjoyed the game."

# game/30_how.rpy:290
translate english faq2_05bf67e5:

    # a1 "Также ты можешь поддержать меня лично на {a=http://www.patreon.com/akabur}www.patreon.com/akabur{/a}"
    a1 "You could also support me personally at {a=http://www.patreon.com/akabur}www.patreon.com/akabur{/a}"

# game/30_how.rpy:291
translate english faq2_722f56af:

    # a1 "Еще один способ поддержать меня это перейти по этой ссылке: {a=http://akabur.hentaiunited.com}akabur.hentaiunited.com{/a}."
    a1 "Another way to support me though would be to follow this link: {a=http://akabur.hentaiunited.com}akabur.hentaiunited.com{/a}."

# game/30_how.rpy:292
translate english faq2_3c9baed1:

    # a1 "У таких независимых художников как я каждый бакс на счету..."
    a1 "For an independent artist like myself every buck counts..."

# game/30_how.rpy:293
translate english faq2_a4228481:

    # a6 "Серьезно. Из-за моего стиля жизни, гребанный банк все еще не может выдать мне кредитную карту..."
    a6 "I'm serious. Because of the way I make a living the damn bank won't even issue me a creditcard still..."

# game/30_how.rpy:315
translate english faq2_c164d665:

    # a1 "Ну, перейти по этой ссылке: {a=http://akabur.hentaiunited.com}akabur.hentaiunited.com{/a} и подписаться. Или посетить мой сайт: {a=http://akabur.com}akabur.com{/a}."
    a1 "Well, follow this link: {a=http://akabur.hentaiunited.com}akabur.hentaiunited.com{/a} and subscribe. Or visit my site: {a=http://akabur.com}akabur.com{/a}."

# game/30_how.rpy:316
translate english faq2_305d689c:

    # a1 "Еще, конечно есть Patreon {a=http://www.patreon.com/akabur}www.patreon.com/akabur{/a}.\nи {a=https://twitter.com/AKABUR}мой twitter{/a}."
    a1 "There is also Patreon of course {a=http://www.patreon.com/akabur}www.patreon.com/akabur{/a}.\nAnd {a=https://twitter.com/AKABUR}my twitter{/a}."

# game/30_how.rpy:319
translate english faq2_42b4207e:

    # a1 "Если у тебя есть вопрос, который не вошел в этот \"F.A.Q.\", не стесняйся и пиши: akaburfake2@yahoo.com"
    a1 "If you have a question that is not covered in this \"F.A.Q.\", feel free to email it to me: akaburfake2@yahoo.com"

# game/30_how.rpy:320
translate english faq2_08d1b348:

    # a1 "Или оставь вопрос здесь: {a=http://ask.fm/AKABUR}ask.fm/AKABUR{/a}"
    a1 "Or leave your question here: {a=http://ask.fm/AKABUR}ask.fm/AKABUR{/a}"

# game/30_how.rpy:323
translate english faq2_89f5f0c5:

    # a1 "Эта игра тестировалась много много много раз, но лучшими тестерами всегда были и остаются игроки."
    a1 "This game has been tested many many many times, but the best testers are always the players."

# game/30_how.rpy:324
translate english faq2_18f2162c:

    # a1 "Так что, если вы встретили баг - пишите мне (akaburfake2@yahoo.com) и я исправлю проблеиу в следущей версии игры."
    a1 "So if you did encounter a bug, typo or even a grammatical error - feel free to contact me (akaburfake2@yahoo.com) and I will fix the problem in the next version of the game."

# game/30_how.rpy:327
translate english faq2_2cc681e3:

    # a1 "Никто не помогал мне! Я сделал все сам!"
    a1 "Nobody helped me! I did everything myself!"

# game/30_how.rpy:328
translate english faq2_d97a25a1:

    # a1 "Я сам написал все скрипты, нарисовал все арты, и сыграл всю музыку!"
    a1 "I wrote all the scripts, created all the art, and composed all the music!"

# game/30_how.rpy:329
translate english faq2_687c78af:

    # a7 "Я! {size=+3}Я! {size=+3}Я создал все! {size=+3}Я!{/size}"
    a7 "Me! {size=+3}Me! {size=+3}I created everything! {size=+3}me!{/size}"

# game/30_how.rpy:330
translate english faq2_34cdd6f6:

    # a2 "Хех..."
    a2 "Heh..."

# game/30_how.rpy:331
translate english faq2_1dde8a96:

    # a1 "Ну, по правде, я сделал большую часть работу. Но мне много помогали."
    a1 "Well, in truth I did most of the work. But I had a lot of help also."

# game/30_how.rpy:332
translate english faq2_46f2af4a:

    # a1 "Мой друг и коллега Dahr обеспечил меня благородно (и бесплатно) разными художествами (помимо всего прочего)."
    a1 "My friend and colleague Dahr provided me graciously (and free of charge) with a lot sorts of additional art (among other things)."

# game/30_how.rpy:333
translate english faq2_15a8a54b:

    # a1 "Не бойтись кинуть парню монетки или две на {a=http://www.patreon.com/DAHR}www.patreon.com/DAHR{/a}"
    a1 "Feel free to throw a coin or two the man's way at {a=http://www.patreon.com/DAHR}www.patreon.com/DAHR{/a}"

# game/30_how.rpy:334
translate english faq2_dcfc8d09:

    # a1 "Xaljio всегда был рядом, когда у меня были проблемы с коддингом. (частенько). Посетите его страничку - {a=http://www.patreon.com/xaljio}www.patreon.com/xaljio{/a}"
    a1 "Xaljio was always there for me whenever I needed help with coding. (quite often). Fell free to visit his page at {a=http://www.patreon.com/xaljio}www.patreon.com/xaljio{/a}"

# game/30_how.rpy:335
translate english faq2_d10f66f4:

    # a1 "И, конечно, сообщества на patreon и hentaiunited. Парни, вы шикарны."
    a1 "And of course patreon and hentaiunited community. You guys are awesome."

# game/30_how.rpy:336
translate english faq2_3de1696c:

    # a1 "Спасибо вам за моральную и финансовую поддержку разработки этой игры. Вы, ребята, делаете этот мир чуточку лучше."
    a1 "Thank you for being so supportive and for financing the development of this game. You guys made this world a bit better place."

# game/30_how.rpy:339
translate english faq2_a430831e:

    # a1 "Как я уже говорил, я не стою на месте."
    a1 "Like I said I don't stand still."

# game/30_how.rpy:340
translate english faq2_03c9f5af:

    # a1 "И уже начал разработку нового проекта. {size=-4}(\"Тренер Принцессы Золотое Издание\"){/size}"
    a1 "I jumped on my next project. {size=-4}(\"Princess Trainer Gold Edition\"){/size}"

# game/30_how.rpy:341
translate english faq2_6b59442f:

    # a1 "..."
    a1 "..."

# game/30_how.rpy:342
translate english faq2_6b59442f_1:

    # a1 "..."
    a1 "..."

# game/30_how.rpy:343
translate english faq2_98203ef9:

    # a1 "Но если тебе понравилась конкретно эта часть, ты можешь найти группу программистов-энтузиастов занимающихся модифицированием..."
    a1 "But if you liked this game in particular, you can go find the group of programmers-enthusiasts who are doing modifications."

# game/30_how.rpy:344
translate english faq2_8dc207a9:

    # a1 "А возможно и продолжением клиента."
    a1 "And maybe some new content."

# game/30_how.rpy:345
translate english faq2_f68937eb:

    # a6 "И хоть я и против этого..."
    a6 "Even if I am against it..."

# game/30_how.rpy:346
translate english faq2_820b0f8e:

    # a2 "Но от них не избавиться."
    a2 "You can't get rid of them."

# game/30_how.rpy:348
translate english faq2_6b59442f_2:

    # a1 "..."
    a1 "..."

# game/30_how.rpy:349
translate english faq2_6b59442f_3:

    # a1 "..."
    a1 "..."

# game/30_how.rpy:350
translate english faq2_d15e9acd:

    # a5 "Нет."
    a5 "No."

# game/30_how.rpy:352
translate english faq2_1c34cbab:

    # nyor "Странный вопрос, не находишь?"
    nyor "Strange question, don't you think so?"

# game/30_how.rpy:353
translate english faq2_4be3fe79:

    # nyor "И это после того, как я полтора гребанных часа возился, чтобы перевести никому не нужные FAQ и обучалки!"
    nyor "And all this after I spent an hour and a half to translate the guides and F.A.Q. that nobody cares about."

# game/30_how.rpy:354
translate english faq2_2f58e089:

    # nyor "Агрх!"
    nyor "Arhr!"

# game/30_how.rpy:355
translate english faq2_90658731:

    # nyor "А вообще, молодец, что заглянул."
    nyor "Actually, thank you for coming by."

# game/30_how.rpy:356
translate english faq2_5ae84984:

    # nyor "Минута славы."
    nyor "A minute of fame."

# game/30_how.rpy:357
translate english faq2_b2257168:

    # nyor "Перевод для вас пилили \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=14141420}Ave_Fletch{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=15155170}Discordnk90{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=8041771}maniac4a{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=4472572}Rio-Violente{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=16957111}MrFrost1991{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=18304384}babaylolxz{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=15179745}Khan32{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=11908608}sn0rk95{/a}, \ и любимец публики {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=16733487}Nyarkohotep{/a}."
    nyor "The translation was done by \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=14141420}Ave_Fletch{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=15155170}Discordnk90{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=8041771}maniac4a{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=4472572}Rio-Violente{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=16957111}MrFrost1991{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=18304384}babaylolxz{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=15179745}Khan32{/a}, \ {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=11908608}sn0rk95{/a}, \ and the crowd favorite {a=http://pornolab.net/forum/profile.php?mode=viewprofile&u=16733487}Nyarkohotep{/a}."

# game/30_how.rpy:367
translate english faq2_4fc764c0:

    # nyor "Извините, если кого-то забыли :3!"
    nyor "I'm sorry if I forget anybody."

# game/30_how.rpy:368
translate english faq2_9abcfd57:

    # nyor "Не забывайте пользоваться салфетками, ребята, пока-пока!"
    nyor "Don't forget to use the napkins guys, bye-bye!"

# game/30_how.rpy:369
translate english faq2_6b59442f_4:

    # a1 "..."
    a1 "..."

# game/30_how.rpy:370
translate english faq2_3ce94531:

    # a5 "А спросить моего разрешения на перевод, не нужно?"
    a5 "Don't you need to ask my permission for doing whatever you are doing here?"

# game/30_how.rpy:371
translate english faq2_2196eaa4:

    # felix "Ты вроде как ушел..."
    felix "Your sort of quit..."

# game/30_how.rpy:372
translate english faq2_6b59442f_5:

    # a1 "..."
    a1 "..."

# game/30_how.rpy:373
translate english faq2_6d614257:

    # a6 "Это ничего не меняет..."
    a6 "It doesn't change the fact..."

# game/30_how.rpy:374
translate english faq2_8fa90e73:

    # felix "Да конечно..."
    felix "Yeh, sure..."

# game/30_how.rpy:393
translate english devel_225fabed:

    # dev "Итак, вы уже обратили внимание, что это не оригинальная игра Акабура..."
    dev "Ok, I bet you noticed already that this is not the original game by Akabur..."

# game/30_how.rpy:396
translate english devel_5ca6d87f:

    # dev "(facepalm)"
    dev "(facepalm)"

# game/30_how.rpy:397
translate english devel_246e245e:

    # dev "Я так и знал, что нужно давать больше информации общественности..."
    dev "I knew it, that people need more information..."

# game/30_how.rpy:400
translate english devel_991a19f2:

    # dev "..........."
    dev "..........."

# game/30_how.rpy:403
translate english devel_22a5d37e:

    # dev "В рот мне ноги..."
    dev "Fuck my brain..."

# game/30_how.rpy:405
translate english devel_16cf924b:

    # dev "{size=+3}Т.е. вы все по-прежнему считаете, что игру для вас продолжает улучшать Акабур ?{/size}"
    dev "{size=+3}You still think that Akabur continues to improve the game for you?{/size}"

# game/30_how.rpy:406
translate english devel_eb85e19d:

    # dev "И это после того, как он сообщил, что считает ее законченной ?"
    dev "This is after he told everyone, that he considers the game finished."

# game/30_how.rpy:407
translate english devel_5d3e74f8:

    # dev "После того, как он решил никогда ее не переводить на русский ?"
    dev "After he decided to not translate it to Russian or any other language?"

# game/30_how.rpy:408
translate english devel_731692e9:

    # dev "После того, как я написал всю эту гору кода, не говоря уж об остальной команде разработчиков, переводчиков и художников, месяцами бесплатно трудящихся для вас ?"
    dev "Well, after this coding, without speaking about the rest of the developer team, artists and translators, working for months free of charge for you?"

# game/30_how.rpy:409
translate english devel_af71e336:

    # dev "{size=+4}Аргх...{/size}"
    dev "{size=+4}Argh...{/size}"

# game/30_how.rpy:410
translate english devel_2e066e8a:

    # dev "......."
    dev "......."

# game/30_how.rpy:411
translate english devel_11908d6f:

    # dev "Простите, наболело."
    dev "I'm sorry, it's just painfull."

# game/30_how.rpy:413
translate english devel_19a66bea:

    # dev "Итак, для вас трудились :"
    dev "So, I present you the people who made this happen:"

# game/30_how.rpy:414
translate english devel_35ad767c:

    # dev "Главный координатор (встречайте стоя !): {color=4F4F4F}Хан ( Khan ){/color}"
    dev "Chief coordinator (standing ovations!) : {color=4F4F4F}Khan{/color}"

# game/30_how.rpy:415
translate english devel_828990f0:

    # dev "Главный разработчик : {color=00FF00}Дрон (devon12355){/color}"
    dev "Chief devoloper : {color=00FF00}devon12355{/color}"

# game/30_how.rpy:416
translate english devel_9f38a03f:

    # dev "Разработка и ивенты Дафны : {color=7789CA}Феликс{/color}"
    dev "Daphne developer and events : {color=7789CA} {/color}"

# game/30_how.rpy:417
translate english devel_df185580:

    # dev "Поддержка игры (на плаву) : {color=0000FF}Сказочник{/color}"
    dev "Game support (afloat): {color=0000FF} Skazochnik {/color}"

# game/30_how.rpy:418
translate english devel_3cd32fed:

    # dev "Перевод на английский язык : {color=0089BE}Sleepless{/color}"
    dev "English translation: {color=0089BE}Sleepless{/color}"

# game/30_how.rpy:419
translate english devel_e596fc64:

    # dev "Разработка и обучающие ивенты : {color=FF0000}Nyarkohopter{/color}"
    dev "Training devoloper and events : {color=FF0000}Nyarkohopter{/color}"

# game/30_how.rpy:420
translate english devel_9ff0a49f:

    # dev "Чибики Дафны : {color=2F2F2F}Grending{/color}"
    dev "Chibis for Daphne : {color=2F2F2F}Grending{/color}"

# game/30_how.rpy:421
translate english devel_b8f433be:

    # dev "Дафна : {color=6F6F6F}Zio Dyne{/color}"
    dev "Daphne : {color=6F6F6F}Zio Dyne{/color}"

# game/30_how.rpy:423
translate english devel_bd29ade8:

    # dev "Список особых благодарностей :"
    dev "Special thanks :"

# game/30_how.rpy:424
translate english devel_b37a2e77:

    # dev "{color=0F0F0F}Евгений aka Afar{/color} - за великолепный кодинг и неоценимый вклад в развитие проекта !"
    dev "{color=0F0F0F}Evgeny aka Afar{/color} - for amazing coding and invaluable contribution to the project!"

# game/30_how.rpy:432
translate english devel_924f3274:

    # dev "И несравненная Гермиона Грейнджер в роли офисной шл..."
    dev "And of course the incomparable Hermione Granger as the school whor..."

# game/30_how.rpy:434
translate english devel_f7a4aa47:

    # her "Что-о-о-о ???"
    her "What-t-t-t???"

# game/30_how.rpy:437
translate english devel_1c476a00:

    # dev "Прости, в роли секретут..."
    dev "I'm sorry, as school prost..."

# game/30_how.rpy:442
translate english devel_0312573b:

    # her "А на тебя давно в последний раз подавали в суд\n за половую дискриминацию ?"
    her "When was the last time you someone filed a lawsuit against against you\n for sexual discrimination?"

# game/30_how.rpy:444
translate english devel_3c218069:

    # dev "Бхм. И наша главная офис-леди - мисс Грейнджер."
    dev "Bah. And our main schoolgirl - Miss Hermione Granger."

# game/30_how.rpy:449
translate english devel_b3ef7cc1:

    # her "Так то лучше !"
    her "That's much better!"

# game/30_how.rpy:450
translate english devel_2f3518dc:

    # her "Всем до встречи в игре."
    her "See you in the game."

# game/30_how.rpy:452
translate english devel_d7d60809:

    # dev "Недотрога..."
    dev "Sensitive..."

# game/30_how.rpy:453
translate english devel_8ae58089:

    # dev "Ушла наконец."
    dev "Finally gone."

# game/30_how.rpy:455
translate english devel_34a25555:

    # dev "Итак, приятной игры, друзья !"
    dev "Well, have fun and enjoy the game, friends!"

# game/30_how.rpy:457
translate english devel_cf2f0ac0:

    # dev "А если хотите пообщаться с людьми, которые продолжают совершенствование и расширение игры, милости просим."
    dev "And if you want to have a conversation with the people that continue to enhance and extend the game, you are most welcome to do so."

# game/30_how.rpy:458
translate english devel_a43e78bb:

    # dev "{a=http://skazgames.com/forum/}НАШ ФОРУМ ТУТ{/a}"
    dev "{a=http://skazgames.com/forum/}OUR FORUM IS HERE{/a}"

# game/30_how.rpy:468
translate english forum_64579cee:

    # dev "Итак, перед вами модификация игры, которая развивается независимой (от Акабура) командой разработчиков. Добро пожаловать на {a=http://skazgames.com/forum/}НАШ ФОРУМ{/a}."
    dev "So, what you see before you is a modified version of the game, that's being made independently (without Akabur's participation) by a team of dedicated developers. Welcome to our {a=http://skazgames.com/forum/}FORUM{/a}."

translate english strings:

    # game/30_how.rpy:89
    old "\"Еще бы!\""
    new "\"You bet!\""

    # game/30_how.rpy:89
    old "\"Нет.\""
    new "\"Not interested.\""

    # game/30_how.rpy:286
    old "Как я могу поддержать тебя?"
    new "How can I show my support?"

    # game/30_how.rpy:286
    old "Как оставаться в курсе?"
    new "How to stay tuned?"

    # game/30_how.rpy:286
    old "У меня есть другой вопрос."
    new "I have another question."

    # game/30_how.rpy:286
    old "Я хочу сообщить о баге/ошибке."
    new "I want to report a bug/error."

    # game/30_how.rpy:286
    old "Кто помог тебе создавать игру?"
    new "Who helped you create this game?"

    # game/30_how.rpy:286
    old "Будет ли продолжение этой игры?"
    new "Will be there any extension of this game?"

    # game/30_how.rpy:286
    old "Можно взломать и перевести эту игру?"
    new "Is it OK to hack and translate this game?"

    # game/30_how.rpy:286
    old "Неважно. Выпусти меня отсюда!"
    new "Nevermind. Let me out of here!"

    # game/30_how.rpy:394
    old "Что ???"
    new "What???"

    # game/30_how.rpy:394
    old "Это же шутка ?"
    new "It's a joke, right?"

    # game/30_how.rpy:394
    old "А разве ты не Акабур ?"
    new "You are not Akabur, aren't you?"

# TODO: Translation updated at 2015-07-21 15:21

# game/30_how.rpy:251
translate english abouttrainer_770ca530:

    # dev "Итак, сейчас перед вами \"Тренер Ведьм: Русская редакция\"."
    dev "Alright, what you see before you is \"Witches Trainer: Russian Editon\"."

# game/30_how.rpy:252
translate english abouttrainer_2511b212:

    # dev "Наша дружная команда взяла за основу небольшую, но очень приятную игру от Акабура."
    dev "Our friendly team, took as a base the small, but very pleasant game from Akabur."

# game/30_how.rpy:253
translate english abouttrainer_dd0809a4:

    # dev "И делает на основе ее идей и контента нечто новое."
    dev "And is building on its main ideas and content something completly new."

# game/30_how.rpy:254
translate english abouttrainer_b464ab67:

    # dev "Новые ивенты и персонажи, система гардероба и газетное издательство... Все это только у нас."
    dev "New events and characters, a wardrobe sytem and a newspaper publishing house... All of this only for you."

# game/30_how.rpy:255
translate english abouttrainer_b1f96825:

    # dev "Кроме того, мы не считаем проект законченным и постоянно совершенствуем его."
    dev "Moreover, we don't consider the project finished and we are constantly trying to improve it."

# game/30_how.rpy:256
translate english abouttrainer_30fefcb6:

    # dev "А еще, перевели игру на русский язык, чтобы русским людям было приятно играть, и сделали мультиязычную версию."
    dev "And we implimented a multilanguage support and translation of the game, so everybody will be able to enjoy it."

# game/30_how.rpy:257
translate english abouttrainer_9c80e5d8:

    # dev "Помните, что мы делаем это совершенно бесплатно. Для собственного и вашего удовольствия."
    dev "Remember we are doing it completely free of charge. For our and your enjoyment."

# game/30_how.rpy:258
translate english abouttrainer_d1443c28:

    # dev "Все пожертвования в фонд развития игры пойдут не на пиво разработчикам, а на различные нужды проекта."
    dev "All your donations will not be spend on bear for developers, but for the actual needs of the project."

# game/30_how.rpy:271
translate english devel_5c091bde:

    # dev "Итак, вы уже обратили внимание, что это не оригинальная игра Акабура..."
    dev "Ok, I think you already noticed, that this is not the original game by Akabur..."

# game/30_how.rpy:274
translate english devel_0733a614:

    # dev "(facepalm)"
    dev "(facepalm)"

# game/30_how.rpy:275
translate english devel_1b8ae4f8:

    # dev "Я так и знал, что нужно давать больше информации общественности..."
    dev "I knew it, that people should be given more information..."

# game/30_how.rpy:278
translate english devel_d73f02af:

    # dev "..........."
    dev "..........."

# game/30_how.rpy:281
translate english devel_33de5845:

    # dev "В рот мне ноги..."
    dev "Fuck my brain..."

# game/30_how.rpy:283
translate english devel_b92cbe9a:

    # dev "{size=+3}Т.е. вы все по-прежнему считаете, что игру для вас продолжает улучшать Акабур ?{/size}"
    dev "{size=+3}You still think that is Akabur who continues to improve the game for you?{/size}"

# game/30_how.rpy:284
translate english devel_a2b51fa9:

    # dev "И это после того, как он сообщил, что считает ее законченной ?"
    dev "And all this after he told everyone, that he considers the game finished."

# game/30_how.rpy:285
translate english devel_c6369ff8:

    # dev "После того, как он решил никогда ее не переводить на русский ?"
    dev "After he decided to not translate it to Russian or any other language?"

# game/30_how.rpy:286
translate english devel_84d486ea:

    # dev "После того, как я написал всю эту гору кода, не говоря уж об остальной команде разработчиков, переводчиков и художников, месяцами бесплатно трудящихся для вас ?"
    dev "Well, after all this coding, without speaking about the rest of the developer team, artists and translators, working for months for free jusr for you?"

# game/30_how.rpy:287
translate english devel_abe231fb:

    # dev "{size=+4}Аргх...{/size}"
    dev "{size=+4}Argh...{/size}"

# game/30_how.rpy:288
translate english devel_56f34156:

    # dev "......."
    dev "......."

# game/30_how.rpy:289
translate english devel_9d9f0e63:

    # dev "Простите, наболело."
    dev "I'm sorry, it's just painfull."

# game/30_how.rpy:291
translate english devel_d328bf1c:

    # dev "Итак, для вас трудились :"
    dev "So, I present you the people who made this game happen:"

# game/30_how.rpy:292
translate english devel_56d64cac:

    # dev "Главный координатор (встречайте стоя !): {color=4F4F4F}Хан ( Khan ){/color}"
    dev "Chief coordinator (standing ovations!): {color=4F4F4F}Khan{/color}"

# game/30_how.rpy:293
translate english devel_00a807be:

    # dev "Главный разработчик : {color=00FF00}Дрон (devon){/color}"
    dev "Chief devoloper: {color=00FF00}devon{/color}"

# game/30_how.rpy:294
translate english devel_70dc2291:

    # dev "Разработка и ивенты Дафны : {color=7789CA}Феликс{/color}"
    dev "Developer of Daphne and her events : {color=7789CA}Феликс{/color}"

# game/30_how.rpy:295
translate english devel_ca307d79:

    # dev "Поддержка игры (на плаву) : {color=0000FF}Сказочник{/color}"
    dev "Game support (afloat): {color=0000FF}Сказочник{/color}"

# game/30_how.rpy:296
translate english devel_081a1b4b:

    # dev "Перевод на английский язык : {color=0089BE}Хан и Sezt{/color}"
    dev "English translation: {color=0089BE}Khan и Sezt{/color}"

# game/30_how.rpy:297
translate english devel_17cbd3ca:

    # dev "Разработка и обучающие ивенты : {color=FF0000}Nyarkohopter{/color}"
    dev "Devoloping and training events: {color=FF0000}Nyarkohopter{/color}"

# game/30_how.rpy:298
translate english devel_2714e354:

    # dev "Чибики Дафны : {color=2F2F2F}Grending{/color}"
    dev "Chibis for Daphne: {color=2F2F2F}Grending{/color}"

# game/30_how.rpy:299
translate english devel_e492ec03:

    # dev "Дафна : {color=6F6F6F}Zio Dyne{/color}"
    dev "Daphne: {color=6F6F6F}Zio Dyne{/color}"

# game/30_how.rpy:301
translate english devel_0d7e350e:

    # dev "{color=0F0F0F}Eskelsama{/color} - за великолепный кодинг и неоценимый вклад в развитие проекта !"
    dev "{color=0F0F0F}Eskelsama{/color} - for amazing coding and invaluable contribution to the project!"

# game/30_how.rpy:309
translate english devel_95789c32:

    # dev "И несравненная Гермиона Грейнджер в роли офисной шл..."
    dev "And of course the incomparable Hermione Granger as the school whor..."

# game/30_how.rpy:314
translate english devel_7758cfe4:

    # dev "Прости, в роли секретут..."
    dev "What-t-t-t???"

# game/30_how.rpy:321
translate english devel_14d0b826:

    # dev "Бхм. И наша главная офис-леди - мисс Грейнджер."
    dev "Bah. And our main schoolgirl - Miss Hermione Granger."

# game/30_how.rpy:329
translate english devel_e594dfd2:

    # dev "Недотрога..."
    dev "Sensitive..."

# game/30_how.rpy:330
translate english devel_41e139b7:

    # dev "Ушла наконец."
    dev "Finally gone."

# game/30_how.rpy:332
translate english devel_1ac08b57:

    # dev "Итак, приятной игры, друзья !"
    dev "Well, have fun and enjoy the game, friends!"

# game/30_how.rpy:334
translate english devel_0e6b5f9f:

    # dev "А если хотите пообщаться с людьми, которые продолжают совершенствование и расширение игры, милости просим."
    dev "And if you want to have a conversation with the people that continue to enhance and extend the game, you are most welcome to do so."

# game/30_how.rpy:335
translate english devel_fb3f35e0:

    # dev "{a=http://skazgames.com/forum/}НАШ ФОРУМ ТУТ{/a}"
    dev "{a=http://skazgames.com/forum/}Our forum is HERE{/a}"

# game/30_how.rpy:345
translate english forum_d54c28dc:

    # dev "Итак, перед вами модификация игры, которая развивается независимой (от Акабура) командой разработчиков. Добро пожаловать на {a=http://skazgames.com/forum/}НАШ ФОРУМ{/a}."
    dev "So, what you see before you is a modified version of the game, that's being made independently (without Akabur's participation) by a team of dedicated developers. Welcome to our {a=http://skazgames.com/forum/}FORUM{/a}."

# TODO: Translation updated at 2015-07-28 10:13

# game/30_how.rpy:300
translate english devel_04769f66:

    # dev "Список особых благодарностей :"
    dev "Special thanks :"

# game/30_how.rpy:301
translate english devel_06d7322a:

    # dev "{color=0F0F0F}Eskelsama{/color} - за великолепный кодинг и неоценимый вклад в развитие проекта !"
    dev "{color=0F0F0F}Eskelsama{/color} - for amazing coding and invaluable contribution to the project!"

