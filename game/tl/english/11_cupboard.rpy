# TODO: Translation updated at 2015-07-13 21:36

# game/11_cupboard.rpy:14
translate english cupboard_f85db927:

    # m "Хм....."
    m "Hm....."

# game/11_cupboard.rpy:15
translate english cupboard_182ff2a4:

    # m "Шкаф..."
    m "A cupboard..."

# game/11_cupboard.rpy:16
translate english cupboard_eba7dd68:

    # m "Я думаю, в нем есть что-то полезное..."
    m "...seems it's full of old junk!"

# game/11_cupboard.rpy:17
translate english cupboard_4e31fb5c:

    # m "Может порыться в нем чуть позже..."
    m "Maybe I should rummage through this one later..."

# game/11_cupboard.rpy:49
translate english menu_cupboard_description_0e8c385f:

    # ">[item._description]"
    ">[item._description]"

# game/11_cupboard.rpy:51
translate english menu_cupboard_description_4f4386d1:

    # "> Вы пробовали понюхать эти духи при получении, и нашли их отвратительными."
    "> You sniffed the delivered perfume and found its scent disgusting."

# game/11_cupboard.rpy:52
translate english menu_cupboard_description_0ba2512c:

    # "> Но может быть вы ошиблись? Вы прыскаете из пузырька..."
    "> May be you are mistaken? You sprayed a little in the air..."

# game/11_cupboard.rpy:53
translate english menu_cupboard_description_31b40d49:

    # "> Какая гадость! Ваше первое впечатление оказалось верным..."
    "> What a garbage! Your first impression was true..."

# game/11_cupboard.rpy:64
translate english cheat_help_277efec1:

    # "ТУРБО-режим включен. Теперь ваши действия будут приносить вам вдвое больше денег и очков факультету Слизерина.\n Шанс на прочтение дополнительной главы вдвое больше."
    "TURBO-mode is enabled. From now on your actions will bring you twice the amount of coins and house points to Slytherin.\n. The chance to read an additional book chapter is doubled."

# game/11_cupboard.rpy:67
translate english cheat_help_78475b8e:

    # "ТУРБО-режим выключен. Теперь ваши действия будут приносить вам обычное количество денег и очков факультету Слизерина.\n Шанс на прочтение дополнительной главы стандартный."
    "TURBO-mode is disabled. From now on your actions will bring you the usual amount of coins and house points to Slytherin.\n. The chance to read an additional book chapter is normal."

# game/11_cupboard.rpy:76
translate english cheat_help_03b65cc2:

    # "Готово можете проверить"
    "Ready. Try it."

# game/11_cupboard.rpy:81
translate english cheat_help_ed10f1a5:

    # "Запрос выполнен"
    "Request granted."

# game/11_cupboard.rpy:92
translate english cheat_help_74874caf:

    # "Прохождение и ответы на частые вопросы можно найти {a=http://skazgames.com/forum/viewtopic.php?f=2&t=30}ЗДЕСЬ{/a}. "
    "Walkthrough and FAQ can be found {a=http://skazgames.com/forum/viewtopic.php?f=2&t=30}HERE{/a}."

# game/11_cupboard.rpy:177
translate english rummaging_51f77866:

    # ">Вы роетесь в шкафу..."
    ">You rummage through the cupboard for a while..."

# game/11_cupboard.rpy:186
translate english rummaging_63c659bf:

    # ">Вы нашли какое-то зелье..."
    ">You found some sort of potion..."

# game/11_cupboard.rpy:207
translate english rummaging_54077c21:

    # ">Вы нашли каталог \"Приблуды Дахра\"... \n>Теперь вы можете использовать каталог для заказа товаров с помощью совы."
    ">You found the \"Dahr's Oddities\" catalog...\n>Now you can use the catalog to order goods via owl post."

# game/11_cupboard.rpy:256
translate english rummaging_a72b62f0:

    # ">...Вы не нашли ничего ценного."
    ">...You find nothing of value."

# game/11_cupboard.rpy:282
translate english already_did_5dfb7506:

    # m "Я сегодня уже рылся в шкафу..."
    m "I already did that today..."

translate english strings:

    # game/11_cupboard.rpy:4
    old "- Осмотреть шкаф -"
    new "-Examine the cupboard-"

    # game/11_cupboard.rpy:4
    old "- Рыться в шкафу -"
    new "-Rummage through the cupboard-"

    # game/11_cupboard.rpy:4
    old "{color=#858585}- Рыться в шкафу -{/color}"
    new "{color=#858585}-Rummage through the cupboard-{/color}"

    # game/11_cupboard.rpy:4
    old "- Ваши вещи -"
    new "-Your possessions-"

    # game/11_cupboard.rpy:4
    old "- Священные свитки. Часть I -"
    new "-Sacred scrolls. Volume I-"

    # game/11_cupboard.rpy:4
    old "- Священные свитки. Часть II -"
    new "-Sacred scrolls. Volume II-"

    # game/11_cupboard.rpy:4
    old "- Священные свитки. Часть III -"
    new "-Sacred scrolls. Volume III-"

    # game/11_cupboard.rpy:4
    old "- Священные свитки. Часть IV -"
    new "-Sacred scrolls. Volume IV-"

    # game/11_cupboard.rpy:4
    old "- Священные свитки. Часть V -"
    new "-Sacred scrolls. Volume V-"

    # game/11_cupboard.rpy:61
    old "Включить ТУРБО-режим"
    new "Enable TURBO-mode"

    # game/11_cupboard.rpy:61
    old "Выключить ТУРБО-режим"
    new "Disable TURBO-mode"

    # game/11_cupboard.rpy:61
    old "ЧИТ: +100 очков Слизерину"
    new "CHEAT: +100 house points to Slytherin"

    # game/11_cupboard.rpy:61
    old "ЧИТ: Гермиона не злится на вас"
    new "CHEAT: Hermione is no longer angry at you"

    # game/11_cupboard.rpy:61
    old "ЧИТ: Дафна не гневается на вас"
    new "CHEAT: Daphne is no longer angry at you"

    # game/11_cupboard.rpy:61
    old "ЧИТ: +100 галеонов"
    new "CHEAT: +100 galleons"

    # game/11_cupboard.rpy:61
    old "Прохождение"
    new "Walkthrough"

# TODO: Translation updated at 2015-07-21 15:21

# game/11_cupboard.rpy:140
translate english menu_cupboard_description_wrd_0e8c385f:

    # ">[item._description]"
    ">[item._description]"

translate english strings:

    # game/11_cupboard.rpy:4
    old "- Вещи -"
    new "-Items-"

    # game/11_cupboard.rpy:34
    old "- Одежда для подарков -"
    new "-Clothes as a Gift"

    # game/11_cupboard.rpy:34
    old "- Вещи напрокат -"
    new "-Rented Clothes"

    # game/11_cupboard.rpy:34
    old "- Помощь -"
    new "-Help-"

