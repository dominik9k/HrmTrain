...
if IsRunNumber(1):
    if event._finishCount>4: # Заглушка, нужно будет снять когда будут написаны последующие ивенты
...
label dap_request_01_complete:
    ...
    if IsRunNumber(1):
        ...
        $event.Finalize("daphne_pre_menu") #Тупость - без daphne_pre_menu Дафна не уходит, а с ним уходит 2 раза.
#Вместо daphne_pre_menu можно запихнуть daphne_goout... Но это как вариант.
        $daphne.whoring += 1
    elif IsRunNumber(2):
        ...
        $event.Finalize("daphne_pre_menu")
        $daphne.whoring += 1
#Дальше, Дафна постепенно делает все(не слешком заигрывайся тут, это начало Хд), что бы парни голосовали за неё
