...
label dap_request_03_complete:
    ...
    if IsRunNumber(1):
        ...
        play music "music/Music for Manatees.mp3"
        $hero("Гхм....// Видите ли мисс Гринграсс, профессор Северус, так же хотел заняться вашим обучением...// Но не надеялся, что вы сможете чего-то достичь...// И по этому, отправил вас ко мне...// А теперь как видите, спохватился...// Но вы не переживайте...// Я поговорю с ним, по этому поводу...") #В след. версию добавить рамсы с Северусом по этому поводу, давненько их не было.
        ...
        if item.Name=="mag3" > 0:
            ...
            $daphne("Эм....// Ну....// Профессор, я....// В общем я не смогла сделать это...")
            $hero("Хм... Что же вас смутило в это раз, мисс?")
            $daphne("Сэр, когда я вам говорила, что не обращала внимания на реакцию других студентов...// Это была ложь....// Я не могу выбросить из головы это ужасное чувство стыда....// Которое поглощает меня целиком....// И эти крики, тыканье пальцами, вроде «Посмотри, в чем она одета»....// Я так не могу, профессор....// Не могу отбросить ту мысль, что мне еще раз придется все это испытать...")
        ...
        if item.Name=="mag4" > 0:
            ...
            $daphne("Да, сэр...// Выполнила...")
            $hero("Великолепно!// Что ж... может вы порадуете меня рассказом?")
            $daphne("Ах, да... Конечно...// Э-э... в общем...// ..........// Я прочитала в журнале, который вы мне дали...// Как симулировать симпатию к эксгибиционизму...")
            $hero("Вы знаете, что значит это слово...?")
            $daphne("Теперь... да.")
            $hero("Ну, и что же...?")
            $daphne("Когда эти двое в очередной раз начали смеяться...// Я просто хлопнула рукой себя по попе, и улыбнулась им...")
            $hero("И как они отреагировали?")
            $daphne(".........// Они смотрели на меня с открытым ртом когда уходила...// Да, ваш совет действительно был полезен...// Это даже немного забавно, смотреть на их недоумение.")
            $hero("Славно мисс Гринграсс....// Я знал что вы оправдаете мои надежды.")
            $daphne("Спасибо, профессор...")
        ...
        $daphne("Несомненно, профессор...")
        $hero("Приятно слышать это от вас, Дафна...// Ну... Возможно вы бы хотели...")
        $daphne("Да, да... Я поняла, сэр...// Садитесь по удобнее.")
        $hero("Хорошо... Можешь начинать.")
        $daphne("Эм... Я, как вы и посоветовали, посмотрела журнал с....// В общем это... это так мерзко.// Мне не хотелось его дальше смотреть...// Но....// Ваши советы, профессор... они всегда были ценными, и....// Понимала я их только тогда, когда уже было поздно...")
        $hero("Какой же вывод вы сделали, мисс Гринграсс?")
        $daphne("Э-э....// Что вы как бы то ни было, всегда оказываетесь правы, сэр...")
        $hero("#(Ммм... Хорошо...)")
        $daphne("Что нужно прислушиваться к вашим советам...")
        $hero("#(Да, да, отлично...)// И стало быть....")
        $daphne("Слушаться вас, профессор.")
        $hero("#(Ну наконец-то)// Это похвально, мисс...// Я вновь рад тому, что вы поняли это сами.// Эм... А что касается моего задания? Оно выполнено?")
        $daphne("...Да, сэр.// Но... Я предпочла бы не вдаваться в подробности...// Я сделала что вы просили, а теперь можно я пойду?")
        menu:
            "- Расскажите мне, что случилось -":
                $hero("Мисс... Почему мне нужно каждый раз напоминать вам, зачем мы это делаем...// Вы уже столько всего прошли... что мешает вам сейчас?")
                $daphne("........// Эм... Просто...// Тот парень....// Я никогда бы не стала....")
                $hero("Как же я смогу вам помочь разобраться, если вы молчите?")
            "- Да, конечно... -":
                $hero("Хорошо можете идти...// Только не говорите мне потом, что нужно было слушаться меня...// Почему все именно так получилось...// Ступайте.")
                $daphne("...........// Не могу... Мне и в правду нужно разобраться в этом...")
                $hero("Ну так расскажите.// Расскажите мне по порядку, как и что случилось.")
                $daphne("............")

        $daphne("Ладно....// Эм... Я видела в том журнале что вы мне дали...// Как девушки заигрывали с парнями...// И я решила сделать так же...// Просто попробовать...// Посмотреть парень себя поведет...// Я не думала....")
        $hero("Расслабьтесь, мисс...// Давайте по порядку.")
        $daphne("Хорошо...// В общем я подошла к парню который сидел на скамейке в раздевалке...// И делала те штуки, которые.... из журнала...")
        $hero("Из журнала!?")
        $daphne("Да... Гладила его по... в...")
        $hero("Я понял, мисс....")
        $daphne("Кажется ему были приятны...// Мои движения...")
        $hero("Вы это поняли!?")
        $daphne("Он тяжело дышал, и смотрел мне в глаза...// Я... я не знаю что произошло...// Я не заметила как он достал эту штуку....// Я не понимала что происходит...// Она была... была огромная...// Он сказал, что бы я обхватила его...// Гхм... Я хотела уйти... Но держал меня за руку, и тянул её туда...")
        $hero("Вы поддались?")
        $daphne("У меня не было выбора...// Он сильно схватил меня... и....// Я сделала ему рукой, что он просил...// И все...")
        $hero("Вам или ему это понравилось?")
        $daphne("Я не обращала на него внимания....// Если закрыть глаза, то в этом нет ничего такого...")
        $hero("#(Хе-хе, кроме члена в руке)")
        $daphne("Сэр, я сделала что вы просили... и, можно я....")
        $hero("Да конечно, Дафна... Можете быть свободны.")
        $daphne.whoring += 1
        $wtevent.Finalize("daphne_pre_menu")
    elif IsRunNumber(5):
