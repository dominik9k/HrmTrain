...
#    $hermi.Visibility("body+")(".....")
    ...
    $hero  (m, "#(О-о, вот это шлюшка! Обожаю длинноногих блондинок!...)",g9,"#(...а также брюнеток, шатенок и рыжих. Я могу кончить от одного их вида!)",m," #(Вот только сиськи маловаты. Может, их подрастить заклинанием?..)", 
        ...
        m, "Здравствуйте, мисс э-э...// #(Великие пески, почему он не сказал как ее зовут?).") 
...
# После того, как будет получена информация о выпивке Дафны ивент заканчивается. А так с интервалом в 2 дня при попытке поговорить возобновляется.
    ...
    if hermi.whoring<12:
        ...
        if end.IsEnding(const_ENDING_STRONG_GIRL):
            ...
            $hero  ("Очень смешно!") 
            $hermi("~body_186.png// Тогда посмейтесь, сэр. А я отправлю сову мистеру и миссис Гринрасс, с письмом о том, что вы выведывали непристойную информацию об их дочери.") 
...
label daphne_pre_finish: #LV.1 (Whoring = 0 - 2)
    ...
    if IsNextRun(): # Начиная со второго посещения Дафна в форме чирлидера
        ...
        $daphne.ItemsCustomize(update={"combi:cheer_topbase", "skirt:cheer_long"}, chibi="b")
...
#    label daphne_pre_finish_menu:
    if event._finishCount==0: # Первый раз
        ...
        $daphne("~46 w0 1 def// Признаюсь, я изначально думала, что вы недостойны, чтобы учить меня - наследницу древнего рода.") 
        $hero  ("Хм...")
        ...
        $hero  (m, "В следующий раз, мисс. Жду вас через два дня.// А пока для вас небольшой подарок.") 
        $daphne("~55 00 1 def// .........") 
        $event.Finalize()
...
#        $daphne.ItemsCustomize(update={"hair"})
        ...
        $daphne("~55 w0 1 ang// О, сэр, все так быстро... я вообще-то не уверена,... я никогда до сих пор... Это нужно отменить!") 
        ...
        $hero("Нет-нет, мисс, у нас будут обычные занятия. Ну почти.//" 
            "И уверен, в конце, вы получите столько удовольствия, сколько не получали за свою жизнь. #(Хе-хе)")  
        ...
        $hero  ("Отлично, мисс Гринграсс, и все-таки отдохните сегодня. Я вызову вас, чтобы мы могли приступить к нашим занятим.// Думаю, вы заслуживаете подарка.")  
